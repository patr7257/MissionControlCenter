...
- Board owner: `patr7257` (personal USER project; use `--owner patr7257` for all
  `gh project` commands)
- Project number: `4` (https://github.com/users/patr7257/projects/4)
- Project node ID: `PVT_kwHOCbc9N84BdhEs`
- Status field ID: `PVTSSF_lAHOCbc9N84BdhEszhYCENM` (the BUILT-IN Status field carries the five columns on
  this board; there is no custom Stage field)
- Status option IDs: Backlog `bdccabd2`, Ready `46346dab`, In progress `6a3cbb3a`,
  In review `3f040af5`, Done `708eb0e3`
- Board VIEW: one-time UI step still pending unless done; set the default view's
  layout to Board (it already groups by the built-in Status, so the five columns
  appear; see section 3b, no "Column by" change needed on this board)
